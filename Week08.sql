...
SELECT ProductName,
	DateAdded AS [Stored Value],
	CAST(DateAdded AS DATE) AS [Year, Month, Day],
	CAST(DateAdded AS TIME) AS [Hour, Minutes, Seconds]
	FROM Products;
...
SELECT OrderID,
	CONVERT(VARCHAR, OrderDate, 101) AS [MM/DD/YYYY],
	CONVERT(VARCHAR, OrderDate, 109) AS [Date, Time, Hours, Minutes (AM/PM)],
	CONVERT(VARCHAR, OrderDate, 108) AS [HH/MM/SS (24 hours)]
	FROM Orders;